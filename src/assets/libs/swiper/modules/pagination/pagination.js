    ...
    if (params.type === "bullets") {
      ...
      for (let i = 0; i < numberOfBullets; i += 1) {
        if (params.renderBullet) {
          ...
          paginationHTML += `<${params.bulletElement} className="${params.bulletClass}"></${params.bulletElement}>`;
        }
      }
    }
    if (params.type === "fraction") {
      if (params.renderFraction) {
        ...
        paginationHTML =
          `<span className="${params.currentClass}"></span>` +
          " / " +
          `<span className="${params.totalClass}"></span>`;
      }
    }
    if (params.type === "progressbar") {
      if (params.renderProgressbar) {
        ...
        paginationHTML = `<span className="${params.progressbarFillClass}"></span>`;
      }
    }
    swiper.pagination.bullets = [];
    el.forEach((subEl) => {
