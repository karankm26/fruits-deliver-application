...
import { getMemoById, updateMemo } from "../../features/apiSlice";
...
export default function EditMemo() {
  ...
  const { updateMemoDataLoading, updateMemoDataSuccess, getMemoByIdData } =
    ...
    onSubmit: () => {
      dispatch(
        updateMemo({
          ...
          body: {
            ...values,
            updatedBy: adminData.firstName + " " + adminData.lastName,
          },
        })
      );
      setIsSaved(true);
  ...
  useEffect(() => {
    if (Object.keys(getMemoByIdData).length) {
      setValues({ ...getMemoByIdData });
    }
  }, [getMemoByIdData]);
  ...
  useEffect(() => {
    if (updateMemoDataSuccess && isSaved) {
      navigate("/all-memos");
    }
  }, [updateMemoDataSuccess, isSaved]);
  ...
  const handleEditChange = (e) => {
    const { name } = e.target;
    const isAdmin = [
      "reward.driver_reaches_within",
      "reward.reward_amount",
      "reward.reword_amount_word",
    ];

    const shouldHandleChange =
      adminData.role === "operator" ||
      (adminData.role === "admin" && isAdmin.includes(name));
  ...
  return (
    <Layout>
      <Loader isLoading={updateMemoDataLoading} />
      <div className="row">
          ...
          <div className="page-title-box d-sm-flex align-items-center justify-content-between">
            ...
            <div className="page-title-right">
              <ol className="breadcrumb m-0">
                ...
                <li className="breadcrumb-item active">Edit Memo</li>
              </ol>
            </div>
          </div>
        </div>
