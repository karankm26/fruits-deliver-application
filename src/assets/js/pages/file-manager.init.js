      ...
      var l = e.fileName.includes(".")
          ? (function (e) {
              switch (e) {
                case "png":
                case "jpg":
                  return '<i className="ri-gallery-fill align-bottom text-success"></i>';
                case "pdf":
                  return '<i className="ri-file-pdf-fill align-bottom text-danger"></i>';
                default:
                  return '<i className="ri-file-text-fill align-bottom text-secondary"></i>';
              }
            })(e.fileName.split(".")[1])
          : '<i className="ri-folder-2-fill align-bottom text-warning"></i>',
        i = e.starred ? "active" : "";
      (document.querySelector("#file-list").innerHTML +=
        '<tr>        <td>            <input className="form-control filelist-id" type="hidden" value="' +
        e.id +
        '" id="filelist-' +
        e.id +
        '">            <div className="d-flex align-items-center">                <div className="flex-shrink-0 fs-17 me-2 filelist-icon">' +
        l +
        '</div>                <div className="flex-grow-1 filelist-name">' +
        e.fileName +
        '</div>                <div className="d-none filelist-type">' +
        e.filetype +
        "</div>            </div>        </td>        <td>" +
        e.fileItem +
        '</td>        <td className="filelist-size">' +
        e.fileSize +
        '</td>        <td className="filelist-create">' +
        e.date +
        '</td>        <td>          <div className="d-flex gap-3 justify-content-center">            <button type="button" className="btn btn-ghost-primary btn-icon btn-sm favourite-btn ' +
        i +
        '">                  <i className="ri-star-fill fs-13 align-bottom"></i>            </button>            <div className="dropdown">              <button className="btn btn-light btn-icon btn-sm dropdown" type="button" data-bs-toggle="dropdown" aria-expanded="false">              <i className="ri-more-fill align-bottom"></i>              </button>              <ul className="dropdown-menu dropdown-menu-end">                <li><a className="dropdown-item viewfile-list" href="#">View</a></li>                <li><a className="dropdown-item edit-list" href="#createFileModal" data-bs-toggle="modal" data-edit-id=' +
        e.id +
        ' role="button">Rename</a></li>                <li className="dropdown-divider"></li>                <li><a className="dropdown-item remove-list" href="#removeFileItemModal" data-id=' +
        e.id +
...
getJSON("filemanager-filelist.json", function (e, t) {
  null !== e
    ...
    : loadFileData((allFileList = t));
}),
  favouriteBtn(),
  Array.from(document.querySelectorAll(".file-manager-menu a")).forEach(
    function (i) {
      i.addEventListener("click", function () {
        var e,
          ...
          l =
            ...
            i.querySelector(".file-list-link").innerHTML);
        (document.getElementById("file-list").innerHTML = ""),
          (document.getElementById("filetype-title").innerHTML =
            "My Drive" != l ? l : "Recent file"),
          "My Drive" != l && "Important" != l && "Recents" != l
            ? ((e = allFileList.filter((e) => e.filetype === l)),
              (document.getElementById("folder-list").style.display = "none"))
            : "Important" == l
            ? ((e = allFileList.filter((e) => 1 == e.starred)),
              (document.getElementById("folder-list").style.display = "none"))
            : ((e = allFileList),
      ...
      e.addEventListener("click", function (e) {
        ...
        t ==
          ...
          (document.getElementById("foldername-input").value =
            l.querySelector(".folder-name").innerHTML));
      });
    });
  });
}
    ...
    function (e) {
      ...
      i.checkValidity()
        ? (e.preventDefault(),
          ...
          (folderlisthtml =
            '<div className="col-xxl-3 col-sm-6 folder-card">        <div className="card bg-light shadow-none" id="folder-' +
            l +
            '">            <div className="card-body">                <div className="d-flex mb-1">                    <div className="form-check form-check-danger mb-3 fs-15 flex-grow-1">                        <input className="form-check-input" type="checkbox" value="" id="folderlistCheckbox_' +
            l +
            '">                        <label className="form-check-label" for="folderlistCheckbox_' +
            l +
            '"></label>                    </div>                    <div className="dropdown">                        <button className="btn btn-ghost-primary btn-icon btn-sm dropdown" type="button" data-bs-toggle="dropdown" aria-expanded="false">                            <i className="ri-more-2-fill fs-16 align-bottom"></i>                        </button>                        <ul className="dropdown-menu dropdown-menu-end">                            <li><a className="dropdown-item view-item-btn" href="javascript:void(0);">Open</a></li>                            <li><a className="dropdown-item edit-folder-list" href="#createFolderModal" data-bs-toggle="modal" role="button">Rename</a></li>                            <li><a className="dropdown-item" href="#removeFolderModal" data-bs-toggle="modal" role="button">Delete</a></li>                        </ul>                    </div>                </div>                <div className="text-center">                <div className="mb-2">                    <i className="ri-folder-2-fill align-bottom text-warning display-5"></i>                </div>                    <h6 className="fs-15 folder-name">' +
            t +
            '</h6>                </div>                <div className="hstack mt-4 text-muted">                    <span className="me-auto"><b>0</b> Files</span>                    <span><b>0</b>GB</span>                </div>            </div>        </div>    </div>'),
          "" === t || editFlag
            ? "" !== t &&
              editFlag &&
              ((l = 0),
      ...
      removeProduct.addEventListener("show.bs.modal", function (t) {
        document
          ...
          .addEventListener("click", function (e) {
            t.relatedTarget.closest(".folder-card").remove(),
  ...
  var d = document.getElementsByTagName("body")[0],
    t =
      ...
      Array.from(document.querySelectorAll("#file-list tr")).forEach(function (
        ...
        r.querySelector(".viewfile-list").addEventListener(
          "click",
          function () {
            d.classList.add("file-detail-show"),
