...
export default function EditSubscriptions() {
  ...
  useEffect(() => {
    if (id) dispatch(subscriptionById(id));
  }, [dispatch, id]);

  useEffect(() => {
    if (Object.keys(subscriptionByIdData).length) {
      formik.setValues(subscriptionByIdData);
    }
  }, [subscriptionByIdData]);
  ...
  return (
    <Layout>
      <Loader isLoading={subscriptionByIdDataLoading} />
      <div className="row">
