...
function loadProductData(e) {
  (document.querySelector("#explorecard-list").innerHTML = ""),
    Array.from(e).forEach(function (e, t) {
      var l = e.like ? "active" : "";
      (document.querySelector("#explorecard-list").innerHTML +=
        '<div className="col list-element">            <div className="card explore-box card-animate">                <div className="explore-place-bid-img">                    <input type="hidden" className="form-control" id="' +
        e.id +
        '">                    <div className="d-none">' +
        e.salesType +
        '</div>                    <img src="' +
        e.productImg +
        '" alt="" className="card-img-top explore-img" />                    <div className="bg-overlay"></div>                    <div className="place-bid-btn">                        <a href="#!" className="btn btn-success"><i className="ri-auction-fill align-bottom me-1"></i> Place Bid</a>                    </div>                </div>                <div className="bookmark-icon position-absolute top-0 end-0 p-2">                    <button type="button" className="btn btn-icon ' +
        l +
        '" data-bs-toggle="button" aria-pressed="true"><i className="mdi mdi-cards-heart fs-16"></i></button>                </div>                <div className="card-body">                    <p className="fw-medium mb-0 float-end"><i className="mdi mdi-heart text-danger align-middle"></i> ' +
        e.totalLikes +
        ' </p>                    <h5 className="mb-1"><a href="apps-nft-item-details.html">' +
        e.title +
        '</a></h5>                    <p className="text-muted mb-0">' +
        e.category +
        '</p>                </div>                <div className="card-footer border-top border-top-dashed">                    <div className="d-flex align-items-center">                        <div className="flex-grow-1 fs-14">                            <i className="ri-price-tag-3-fill text-warning align-bottom me-1"></i> Highest: <span className="fw-medium">' +
        e.highBid +
        '</span>                        </div>                        <h5 className="flex-shrink-0 fs-14 text-primary mb-0">' +
        e.price +
...
var searchProductList = document.getElementById("searchProductList"),
  productCategoryInput =
    (searchProductList.addEventListener("keyup", function () {
      ...
      t = e;
      var t,
        e = allproductlist.filter(function (e) {
  ...
  productFileTypeInput =
    ...
    new Choices(document.getElementById("file-type"), { searchEnabled: !1 }));
...
var productSalesInputInput = new Choices(
  ...
  rangeProductPrice =
    ...
    document.getElementById("range-product-price")),
  minCostInput =
    ...
    e &&
      e.noUiSlider.on("update", function (e, t, l, o, n) {
        ...
        for (var d = 1; d < n.length; d++)
          (!g[d] || n[d] - n[d - 1] > c) &&
            ((a[++r] = []), (i[r] = []), (s[r] = [])),
            g[d] && (a[r].push(d), i[r].push(e[d]), s[r].push(n[d]));
        Array.from(a).forEach(function (e, t) {
          for (var l = e.length, o = 0; o < l; o++) {
            ...
            o === l - 1
              ? ((r = 0),
                Array.from(s[t]).forEach(function (e) {
                  r += 1e3 - e;
                }),
                (n = y ? "bottom" : "right"),
                (a = 1e3 - s[t][p ? 0 : l - 1]),
                (r = (m && !y ? 100 : 0) + r / l - a),
                (g[d].innerHTML = i[t].join(u)),
