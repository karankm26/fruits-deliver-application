...
function loadJobListData(e, t) {
  ...
  for (
    ...
    e[i] &&
      ...
      e[i] &&
        (document.querySelector("#job-list").innerHTML +=
          '<div className="card joblist-card">            <div className="card-body">                <div className="d-flex mb-4">                    <div className="avatar-sm">                        <div className="avatar-title bg-light rounded">                            <img src="' +
          e[i].companyLogo +
          '" alt="" className="avatar-xxs companyLogo-img">                        </div>                    </div>                    <div className="ms-3 flex-grow-1">                        <img src="' +
          e[i].coverImg +
          '" alt="" className="d-none cover-img">                        <a href="#!"><h5 className="job-title">' +
          e[i].jobTitle +
          '</h5></a>                        <p className="company-name text-muted mb-0">' +
          e[i].companyName +
          '</p>                    </div>                    <div>                        <button type="button" className="btn btn-ghost-primary btn-icon custom-toggle" data-bs-toggle="button">                            <span className="icon-on"><i className="ri-bookmark-line"></i></span>                            <span className="icon-off"><i className="ri-bookmark-fill"></i></span>                        </button>                    </div>                </div>                <p className="text-muted job-description">' +
          e[i].description +
          "</p>                <div>" +
          a +
          '</div>            </div>            <div className="card-footer border-top-dashed">                <div className="d-flex justify-content-between align-items-center flex-wrap gap-3">                    <div><i className="ri-briefcase-2-line align-bottom me-1"></i> <span className="job-type">' +
          e[i].type +
          '</span></div>                    <div className="d-none"><span className="job-experience">' +
          e[i].experience +
          '</span></div>                    <div><i className="ri-map-pin-2-line align-bottom me-1"></i>  <span className="job-location">' +
          e[i].location +
          '</span></div>                    <div><i className="ri-user-3-line align-bottom me-1"></i> ' +
          e[i].applied +
          '</div>                    <div><i className="ri-time-line align-bottom me-1"></i> <span className="job-postdate">' +
          e[i].postDate +
          '</span></div>                    <div><a href="#!" className="btn btn-primary viewjob-list">View More <i className="ri-arrow-right-line align-bottom ms-1"></i></a></div>                </div>            </div>        </div>');
