...
function loadSellerList(e, t) {
  ...
  for (
    var a, s = (t - 1) * itemsPerPage;
    s < t * itemsPerPage && s < e.length;
    s++
  )
    e[s] &&
      ((a = e[s].trending
        ? '<div className="ribbon ribbon-info ribbon-shape trending-ribbon"><i className="ri-flashlight-fill text-white align-bottom"></i> <span className="trending-ribbon-text">Trending</span></div>'
        : ""),
      (document.getElementById("seller-list").innerHTML +=
        '<div className="col-xl-3 col-lg-6">        <div className="card ribbon-box right overflow-hidden">            <div className="card-body text-center p-4">                ' +
        a +
        '                <img src="' +
        e[s].shop[0].img +
        '" alt="' +
        e[s].shop[0].img_alt +
        '" height="45">                <h5 className="mb-1 mt-4"><a href="apps-ecommerce-seller-details.html" className="link-primary">' +
        e[s].shop[0].name +
        '</a></h5>                <p className="text-muted mb-4">' +
        e[s].seller +
        '</p>                <div className="row justify-content-center">                    <div className="col-lg-8">                        <div id="chart-seller' +
        e[s].id +
        '" data-colors=\'["' +
        e[s].chartColor +
        '"]\'  dir="ltr"></div>                    </div>                </div>                <div className="row mt-4">                    <div className="col-lg-6 border-end-dashed border-end">                        <h5>' +
        e[s].stock +
        '</h5>                        <span className="text-muted">Item Stock</span>                    </div>                    <div className="col-lg-6">                        <h5>' +
        e[s].wallet_balance +
        '</h5>                        <span className="text-muted">Wallet Balance</span>                    </div>                </div>                <div className="mt-4">                    <a href="apps-ecommerce-seller-details.html" className="btn btn-light w-100">View Details</a>                </div>            </div>        </div>    </div>'));
  paginationEvents(),
    pageEvent(e),
    selectedPage(),
    1 == currentPage
